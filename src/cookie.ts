...
function toCookieUnitTime(unit, expires) {
  const num = Number.parseFloat(expires)
...
function cookie(name?, value?, options?) {
  if (staticDocument) {
    let opts, expires, values, result, cookies, keyIndex
    let inserts: any[] = []
      ...
      arrayEach(inserts, (obj) => {
        ...
        if (opts.name) {
          ...
          if (expires) {
            if (isNaN(expires)) {
              // UTCString || Unit
              expires = expires.replace(/^([0-9]+)(y|M|d|H|h|m|s)$/, (text, num, unit) => {
          ...
          arrayEach(['expires', 'path', 'domain', 'secure'], (key) => {
